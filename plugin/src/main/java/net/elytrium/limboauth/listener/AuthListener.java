  ...
  public EventTask onPreLoginEvent(PreLoginEvent event) {
    if (!event.getResult().isForceOfflineMode()) {
      return EventTask.resumeWhenComplete(
          this.plugin.getHybridAuthManager().isPremium(event.getUsername())
              .thenAccept(isPremium -> event.setResult(isPremium ? PreLoginEvent.PreLoginComponentResult.forceOnlineMode() : PreLoginEvent.PreLoginComponentResult.forceOfflineMode()))
