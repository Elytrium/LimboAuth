...
import net.elytrium.limboauth.serialization.CachingComponentSerializer;
...
public class Settings extends YamlSerializable {
  ...
  public static final Settings HEAD = new Settings(new SerializerConfig.Builder()
      .registerSerializer(new ComponentSerializer(Settings::serializer))
      .registerSerializer(new TitleSerializer(Settings::serializer))
      .registerSerializer(new BossBarSerializer(Settings::serializer))
  ...
  public LoadResult reload(Path path) {
    Settings.serializer().clearCache();
    return super.reload(path);
  }

  public static CachingComponentSerializer serializer() {
    return Settings.HEAD == null ? CachingComponentSerializer.LEGACY_AMPERSAND : Settings.HEAD.serializer;
  }

  public final String version = BuildConfig.VERSION;

  @Comment({
      @CommentValue("If you want to auto convert all messages, you need to change serializer while server IS RUNNING and then run this command: /limboauth reload"),
      @CommentValue(),
  ...
  CachingComponentSerializer serializer = CachingComponentSerializer.LEGACY_AMPERSAND; // TODO auto convert on change

  public boolean enableBossbar = true;
  @Comment(@CommentValue("Maximum time for player to authenticate in seconds. If the player stays on the auth limbo for longer than this time, then the player will be kicked"))
  public int authTime = 60;
  ...
  @Comment(@CommentValue("Should be in range [4, 31], not recommended to change if you don't know what you're doing, may significantly affect performance"))
  ...
  public static class PermissionStates {
    ...
    public Commands.PermissionState changePassword = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.commands.destroysession"))
    public Commands.PermissionState destroySession = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.commands.premium"))
    public Commands.PermissionState premium = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.commands.totp"))
    public Commands.PermissionState totp = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.commands.unregister"))
    public Commands.PermissionState unregister = Commands.PermissionState.PERMISSION;

    @Comment(@CommentValue("Permission: limboauth.admin.forcechangepassword"))
    public Commands.PermissionState forceChangePassword = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.admin.forceregister"))
    public Commands.PermissionState forceRegister = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.admin.forceunregister"))
    public Commands.PermissionState forceUnregister = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.admin.reload"))
    public Commands.PermissionState reload = Commands.PermissionState.PERMISSION;
    @Comment(@CommentValue("Permission: limboauth.admin.help"))
    public Commands.PermissionState help = Commands.PermissionState.TRUE;
  }
  ...
  public static class Messages {
    ...
    public Title registerTitle = Title.title(
        Settings.serializer().deserialize("LimboAuth &6>>&f"), Settings.serializer().deserialize("&aPlease, register using &6/register <password> <repeat password>"));
    public Title registerSuccessfulTitle = Title.title(Settings.serializer().deserialize("LimboAuth"), Settings.serializer().deserialize("&aSuccessfully registered!"));
    ...
    @Comment(@CommentValue("Change to \"new password\" if change-password-need-old-pass is set to false"))
    public String changePasswordFirstArgumentName = "old password";
    public String changePasswordSecondArgumentName = "new password";
    public Component changePasswordSuccessful = Settings.serializer().deserialize("LimboAuth &6>>&f &aSuccessfully changed password!");
    @Comment(@CommentValue("Remove the \"<old password>\" part if change-password-need-old-pass is set to false"))
    ...
    @RegisterPlaceholders("TARGET")
    public Component forceChangePasswordNotSuccessful = Settings.serializer()
        .deserialize("LimboAuth &6>>&f &cUnable to change password for &6{TARGET}&c. Most likely this player has never been on this server.");
    @RegisterPlaceholders("TARGET")
