...
class _Alert
{
	...
	{
		return \IPS\core\Alerts\Alert::load( $id1 )->canView( $member );
	}
	...
	{
		return \IPS\core\Alerts\Alert::load( $id1 );
	}
	...
	protected function performRebuild( $offset, $max, $callback )
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_alerts', "alert_content IS NOT NULL", 'alert_id ASC', array( $offset, $max ) ) as $alert )
		{
			$did++;
			...
			try
			{
				...
				{
					$rebuilt = \IPS\Text\Parser::removeImageProxy( $alert['alert_content'], $this->proxyUrl );
				}
				elseif( $callback == 'parseLazyLoad' )
				{
					$rebuilt = \IPS\Text\Parser::parseLazyLoad( $alert['alert_content'], $this->_lazyLoadStatus );
				}
				else
				{
					$rebuilt	= $callback( $alert['alert_content'], \IPS\Member::load( $alert['alert_member_id'] ), FALSE, 'core_Alert' );
				}
			}
			...
			if( $rebuilt !== FALSE )
			{
				\IPS\Db::i()->update( 'core_alerts', array( 'alert_content' => $rebuilt ), array( 'alert_id=?', $alert['alert_id'] ) );
			}
		}
	...
	{
		return \IPS\Db::i()->select( 'COUNT(*) as count', 'core_alerts', "alert_content IS NOT NULL" )->first();
	}
}